...
**The L1 path is open in principle and established locally, not endorsed.** That a
covenant can assert `OpInputCovenantId` on a *sibling* input, and that a one-to-many
split preserves `covenant_id` across all authorised children, were measured on
September 6, 2026 by compiling two variants in Argent and executing them through the
real Kaspa mainnet script engine — see §9 and `docs/design/GITHUB-ISSUE-46-UPDATE.md`.
No core developer has confirmed the reading on
[`kaspanet/kips` issue #46](https://github.com/kaspanet/kips/issues/46), so this
section states a tested reading of the KIPs, not an authoritative one.
...
| Oracle / peg feed | **Yes, measured** | Covenant-lineage construction, compiled and measured; core-dev review pending. Was "No — gating"; see §2, §9 |
